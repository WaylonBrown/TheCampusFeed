angular.module("cfeed").controller "FlaggedPostSectionController", [
  "$scope"
  "$http"
  "$modal"
  "$timeout"
  ...
  ($scope, $http, $modal, $timeout, $rootScope) ->
    ...
    $scope.populatePosts = ->
      $http.get("api/v1/posts/flagged/" + "?page=" + $scope.postOptions.pageNo + "&per_page=" + $scope.postOptions.perPage).success((res) ->
        $scope.postOptions.error = false
    ...
    $scope.setTotal = ->
      $http.get("api/v1/posts/flagged/count").success((res) ->
        $scope.postOptions.error = false
